...
//  ShoppingListIOS
//
//  Copied from https://betterprogramming.pub/using-kotlin-flow-in-swift-3e7b53f559b6
//
...
typealias OnCollect<Output, Failure> = (@escaping OnEach<Output>, @escaping OnCompletion<Failure>) -> ShoppingListCore.Cancellable
...
typealias OnCollect1<T1, Output, Failure> = (T1, @escaping OnEach<Output>, @escaping OnCompletion<Failure>) -> ShoppingListCore.Cancellable
...
class SharedCancellableSubscription: Subscription {
    ...
    var cancellable: ShoppingListCore.Cancellable? {
        didSet {
            if isCancelled {
                    cancellable?.cancel()
                }
